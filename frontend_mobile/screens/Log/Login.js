...
const LoginScreen = ({ navigation }) => {
  ...
  const [sikerultRegisztralniModal, setSikerultRegisztralniModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const sikeresRegisztracioEllenorzes = async () => {
      const modalLathatoe = await AsyncStorage.getItem(
        "sikeresRegisztraciosModal"
      );
    ...
    <View style={Styles.bejelentkezes_Container}>
      <View style={{ marginBottom: 30, alignItems:'center' }}>
        <Text style={{ fontSize: 36, fontWeight: '600' }}>
          <Text style={{ color: "black" }}>Kanyar</Text>
          <Text style={{ color: "#0096FF" }}>GO</Text>
        </Text>
        <Text>Minden kanyar egy új lehetőség!</Text>
      </View>
      ...
      <HibaModal
        ...
        body={
          "Hibás email cím vagy jelszó! Kérjük ellenőrizze, hogy biztosan jól írta-e be az adatait!"
        }
        buttonText={"Bezárás"}
      />
      <SikerModal
