...
const Tanulo_Datumok = ({ atkuld }) => {
  ...
  const adatokBetoltese = async () => {
    try {
      ...
      setBetolt(false);
      setFrissites(false); // Ensure refreshing is set to false
    }
  };
        ...
        {kivalasztottNapOrai.length > 0 ? (
          kivalasztottNapOrai.map((ora) => {
            ...
            return (
              ...
              >
               <Ionicons
                name={ora.ora_tipusID === 1 ? "car-sport-outline" : "school-outline"} 
                size={30}
              ...
              />
                ...
                <Text
                  style={[Styles.kivalasztottDatumOraPerc,{color: "#2EC0F9", fontWeight: "600"}]}
                >{`${oraPerc}`}</Text>
              </View>
            );
          })
